...
k4 = 180000;		// N/m, etwas härter als k3
ma = 270;			// kg
m1 = 7;			// kg
m2 = 9;			// kg
...
M = [ma 0 0 0			// Massenmatrix
   ...
   0 0 m1 0
   0 0 0 m2];
Ds = [d1+d2 a*d1-b*d2 -d1 -d2	// Dämpfungsmatrix
...
A = [zeros(4,4) eye(4,4)		// Systemmatrix
   -inv(M)*K -inv(M)*Ds];
B = [zeros(4,1)			// Eingangsmatrix (mit Kräften)
   inv(M)*Ke];
C = [eye(4,4) zeros(4,4)	// Ausgangsmatrix
   zeros(4,4) 0.05*eye(4,4)];
D = [zeros(4,4)			// Durchgangsmatrix
   zeros(4,4)];
Zweirad = syslin('c',A,B,C);
instants = 0:0.001:1;		// Zeitachse 0 bis 2 s in 1-ms-Schritten
y = csim('step',instants,Zweirad);
//xbasc()				// alte Grafik löschen
xset("font",2,4)		// größerer Font
xtitle("Simulation Zweirad","Zeit t in s","Sprungantwort")
xgrid(32)			// Gitternetzlinien
plot2d(instants',y')		// y' heißt y transponiert
hl = legend('xs','phi','x1','x2','vs','w','v1','v2')
